<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-xl font-bold text-slate-900">Providers</h1>
                <p class="text-sm text-slate-500 mt-0.5">Create staff accounts or generate invitation links for new providers</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.2fr,0.8fr]">
        <x-card :padding="false">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50">
                        ...
                        <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Assigned Services</th>
                    </tr>
                </thead>
                            ...
                            <div class="flex items-center gap-3">
                                @if($provider->profile_photo_path)
                                    <img src="{{ asset('storage/'.$provider->profile_photo_path) }}" alt="{{ $provider->name }}"
                                         class="h-11 w-11 rounded-full object-cover border border-slate-200">
                                @else
                                    <div class="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-sm font-semibold text-slate-500">
                                        {{ strtoupper(substr($provider->name, 0, 1)) }}
                                    </div>
                                @endif
                                <div>
                                    <p class="font-semibold text-slate-900">{{ $provider->name }}</p>
                                    <p class="text-xs text-slate-500">{{ ucfirst($provider->role->value) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-slate-700">{{ $provider->email }}</p>
                            <p class="text-xs text-slate-500">{{ $provider->phone ?: 'No phone on file' }}</p>
                        </td>
                        <td class="px-6 py-4 text-slate-600">
                            {{ $provider->assigned_services_count }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            ...
            <div class="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p class="text-slate-700 font-semibold mb-1">No providers yet</p>
                <p class="text-slate-500 text-sm">Create the first provider account to start assigning services.</p>
            </div>
            @endif
        </x-card>
        ...
        <x-card>
            <div class="mb-5">
                <h2 class="text-base font-semibold text-slate-900">Add Provider</h2>
                <p class="text-sm text-slate-500 mt-0.5">Create an account now or generate a password-reset invitation link to share later.</p>
            </div>
            ...
            <form method="POST" action="{{ route('providers.store') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="bio" name="bio" rows="4"
                              class="block w-full rounded-xl border-slate-300 text-sm focus:border-slate-500 focus:ring-slate-500/30">{{ old('bio') }}</textarea>
