...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-semibold text-gray-950">Book Appointment</h1>
                <p class="mt-0.5 text-sm text-gray-500">Choose a service, then pick a date and time from the calendar.</p>
            </div>
        </div>
    </x-slot>
    ...
    >
        <x-card class="rounded-2xl border-gray-200 shadow-none">
            <form method="POST" action="{{ route('appointments.store') }}" class="space-y-6">
                ...
                <div class="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-gray-50/80 px-5 py-4">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <p class="text-xs font-medium uppercase tracking-[0.24em] text-gray-400">Current selection</p>
                                <p class="mt-2 text-base font-semibold text-gray-950" x-text="selectedService?.name || 'No service available'"></p>
                            </div>
                            <div class="text-right text-sm text-gray-500">
                                <p x-text="selectedService ? `${selectedService.duration_minutes} min` : '--'"></p>
                                <p class="mt-1" x-text="selectedService ? `$${selectedService.price}` : '--'"></p>
                            </div>
                        </div>
                        <p class="mt-3 text-sm text-gray-500">Weekends and past times are locked automatically. Choose a free slot to continue.</p>
                    </div>
                </div>
                ...
                <div x-show="selectedId && !hasLocation" x-cloak class="flex items-center gap-2.5 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500">
                ...
                <div class="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.95fr)]">
                    <section class="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">
                        ...
                        <div x-show="fetchError" x-cloak class="mt-4 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500" x-text="fetchError"></div>
                    ...
                    <aside class="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">
                        ...
                        <div x-show="!loading" x-cloak>
                            <div x-show="!selectedDate" x-cloak class="rounded-2xl border border-dashed border-gray-200 bg-gray-50 px-4 py-6 text-sm text-gray-500">
                            ...
                            <div x-show="selectedDate" x-cloak class="space-y-4">
                                ...
                                <div class="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4">
                                    <p class="text-xs font-medium uppercase tracking-[0.2em] text-gray-400">Appointment</p>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="block w-full resize-none rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 transition-colors focus:border-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-900/10"
                    >{{ old('notes') }}</textarea>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-2">
                    ...
                    <x-primary-button :disabled="$services->isEmpty()" x-bind:disabled="!scheduledAtValue">
...
function bookingCalendar(config) {
    return {
        ...
        get selectedService() {
            return this.services.find((service) => String(service.id) === String(this.selectedId)) || null;
        },

        get selectedServiceDuration() {
            return Number(this.selectedService?.duration_minutes || 30);
        },
        ...
        get currentMonthLabel() {
            ...
            return this.monthCursor.toLocaleDateString(undefined, {
                month: 'long',
                year: 'numeric',
            });
        },
        ...
        get canGoToPreviousMonth() {
            ...
            return this.monthCursor > currentMonth;
        },

        get selectedDateLabel() {
            if (!this.selectedDate) {
                return 'Select a date to unlock the available times.';
            }
        ...
        get calendarDays() {
            ...
            return Array.from({ length: 42 }, (_, index) => {
                ...
                const isFull = inCurrentMonth && !isPast && !isWeekend && this.selectedService
                    ? this.dayHasNoAvailableSlots(dateKey)
                    : false;
        ...
        async fetchBookedSlots() {
            if (!this.selectedService || !this.monthKey) {
                this.bookedSlots = {};
                return;
            }
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error('Unable to load the booking calendar right now.');
                }
        ...
        selectDate(dateKey) {
            if (!this.isDateSelectable(dateKey)) {
                return;
            }
        ...
        isDateSelectable(dateKey) {
            const day = this.calendarDays.find((entry) => entry.date === dateKey);
        ...
        isPastSlot(dateKey, time) {
            return this.toDateTime(dateKey, time) <= new Date();
        },
        ...
        parseScheduledAt(value) {
            ...
            if (!year || !month || !day || time.length !== 5) {
                return null;
            }
